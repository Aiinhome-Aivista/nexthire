...
<body>
    <div class="container">
        ...
        <div class="qualification-section" id="job-types-section">
            ...
            <div class="qualification-item">
                <span
                    id="jobTypesDisplay"><?= htmlspecialchars(empty($job_types) ? 'Not specified' : implode(', ', $job_types)); ?></span>
            </div>
        </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            function updateJobTitleDisplay(title) {
                $('#jobTitleDisplay').text(title || 'Not specified');
            }

            function updateJobTypesDisplay(types) {
                $('#jobTypesDisplay').text(types.length > 0 ? types.join(', ') : 'Not specified');
            }

            function updatePayDisplay(minPay, payPeriod) {
                if (minPay && minPay !== 'null') {
                    const formattedPay = parseFloat(minPay).toLocaleString();
                    $('#payDisplay').text(`₹${formattedPay} ${payPeriod}`);
                } else {
                    $('#payDisplay').text('Not specified');
                }
            }

            function updateRelocationDisplay(isWilling) {
                $('#relocationDisplay').text(isWilling ? 'Willing to relocate' : 'Not willing to relocate');
            }

            function updateRemoteDisplay(setting) {
                $('#remoteDisplay').text(setting || 'Not specified');
            }
            ...
            $('#preferenceModal').on('show.bs.modal', function (event) {
                ...
                switch (modalType) {
                    ...
                    case 'pay':
                        ...
                        const currentPayPeriod = button.data('current-pay-period');
                        ...
                        break;
                    case 'relocation':
                        const currentWillingness = button.data('current-willingness');
                        modal.find('.modal-title').text('Edit Relocation Preference');
                        modalHtml = `
                            <p>Are you willing to relocate?</p>
                            <form id="dynamicForm">
                                <div class="form-check checkbox-group">
                                    <input type="checkbox" class="form-check-input" id="relocationCheckbox" name="is_willing_to_relocate" ${currentWillingness === 'true' ? 'checked' : ''}>
            ...
            $('#saveButton').on('click', function (e) {
                ...
                switch (modalType) {
                    ...
                    case 'relocation':
                        ...
                        callback = () => updateRelocationDisplay(dataToSend.is_willing_to_relocate === 'true');
                        break;
                    case 'work_setting':
